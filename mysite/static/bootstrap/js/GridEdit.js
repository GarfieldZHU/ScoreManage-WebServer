...
function GetTableData(table){
    ...
    for(var j=0; j<tableData.length; j++){
        var item = new Object();
        item.Num = tableData[j]['Num'];
        item.Name = tableData[j]['Name'];
        item.Type = tableData[j]['Type'];
        item.Gender = tableData[j]['Gender'];
        item.Age = tableData[j]['Age'];
        item.Passwd = tableData[j]['Passwd'];
...
function GetCourseTableData(table){
    var tableData = new Array();
    var result = new Array();
    //alert("rows" + table.rows.length);
    ...
    for(var j=0; j<tableData.length; j++){
        ...
        item.Num = tableData[j]['Num'];
        item.Name = tableData[j]['Name'];
        item.Teacher = tableData[j]['Teacher'];
        item.Tid = tableData[j]['Tid'];
        item.Credit = tableData[j]['Credit'];
        item.Semester = tableData[j]['Semester'];
        item.Period = tableData[j]['Period'];
...
//alert(rowData["Name"], rowData["Value"])
